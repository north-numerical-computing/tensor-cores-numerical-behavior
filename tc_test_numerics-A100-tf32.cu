...
int main(int argc, char** argv){
  ...
  printheader(outfile, "A. Support for subnormal numbers");// ;
   ...
   printitem(outfile, "*) TF32 subnormals in input (binary32 mode)");
  host_reset(h_a, h_b, h_c);
  h_a[0] = minsubnormaltf32;
  h_b[0] = ldexp(1, 10);
  ...
  printpass(outfile, h_c[0] == minsubnormal32);
   ...
   printitem(outfile, "*) TF32/Binary32 subnormals in output (binary32 mode)");
  host_reset(h_a, h_b, h_c);
  h_a[0] = ldexp(1., -126);
  h_b[0] = ldexp(1., -1);
  wmma_init_run (h_a, h_b, h_c, d_a, d_b, d_c, false);
